...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("meme")
        .setDescription("Çeşitli Memelere Göz At")
    ...
    async execute(interaction) {
        ...
        async function redditMeme() {
            await fetch('https://www.reddit.com/r/memes/random/.json').then(async res => {
                ...
                return interaction.reply({ embeds: [embed.setTitle(title).setImage(url).setURL(url).setColor("#2f3136").setFooter({ text: "Made by ❤️ ArviS#0011" })] });
            });
        }
        ...
        async function giphyMeme() {
            await fetch('https://api.giphy.com/v1/gifs/random?api_key=lYBciWOvaOhVPNzGy4J8mUXNpC53YNbz&tag=&rating=g').then(async res => {
                ...
                return interaction.reply({
                    embeds: [embed.setTitle(`${title}`).setImage(`${url}`).setURL(link).setColor("#2f3136").setFooter({ text: "Made by ❤️ ArviS#0011" })],
                });
            });
        }
